...
/* USER CODE BEGIN PFP */
void rx_can_irq(CAN_HandleTypeDef *pcan);
void tx_can_irq(CAN_HandleTypeDef *pcan);
/* USER CODE END PFP */
...
int main(void)
{
  ...
  if (HAL_CAN_RegisterCallback(&hcan1, HAL_CAN_RX_FIFO0_MSG_PENDING_CB_ID, rx_can_irq)) {
    Error_Handler();
  }

  if (HAL_CAN_RegisterCallback(&hcan2, HAL_CAN_TX_MAILBOX0_COMPLETE_CB_ID, tx_can_irq)) {
    Error_Handler();
  }
